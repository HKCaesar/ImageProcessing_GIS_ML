import os
from org.saarthak.RasterFunctions import vectors_to_raster, write_geotiff
...
write_geotiff(output_fname, classification, geo_transform, projection)

shapefiles = [os.path.join(validation_data_path, "%s.shp"%c) for c in classes]
verification_pixels = vectors_to_raster(shapefiles, rows, cols, geo_transform, projection)
